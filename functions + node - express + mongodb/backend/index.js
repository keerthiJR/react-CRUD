...
//Get method
app.get('/', (req, res) => {
    mydatabase.collection('batch3').find().toArray((err, result) => {
        if (err) throw err
        res.send(result)
    })
})
...
//     mydatabase.collection('batch3').find(somecond).toArray((err,result)=>{
//         if(err) throw err
//         res.send(result)
//     })
//    })
...
app.post('/post', (req, res) => {
    mydatabase.collection('batch3').insertOne(req.body, (err) => {
        if (err) throw err
...
app.post('/postreact', (req, res) => {
    ...
    mydatabase.collection('batch3').insertOne(abc, (err) => {
        if (err) throw err
        res.send("Data Inserted successfully")
    })
})
...
// app.put('/put', (req, res) => {
//     mydatabase.collection('batch3').findOneAndUpdate(
//         { "stud_id": req.body.id },
...
app.patch('/patch',(req,res)=>{
    mydatabase.collection('batch3').update(
        {"stud_id": req.body.id},
        {$set: {"stud_name":req.body.name}},
        {upsert:true},//if id exists it will update or it create new one
        (err)=>{
...
//     mydatabase.collection('batch3').findOneAndDelete(abc, (err) => {
//         if (err) throw err
...
app.delete('/delete',(req,res)=>{
    ...
    mydatabase.collection('batch3').findOneAndDelete({"stud_id":req.body.id},(err)=>{
        if(err) throw err
        res.send("data deleted successfully")
...
app.delete('/delete/:myid',(req,res)=>{
    mydatabase.collection('batch3').findOneAndDelete({"stud_id":req.params.myid},(err)=>{
        if(err) throw err
        res.send("data deleted successfully2")
        console.log(res)
    })
})